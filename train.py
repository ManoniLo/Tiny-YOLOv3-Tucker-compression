...
def main(args):
    ...
    assert (
        input_shape[0] % 32 == 0 and input_shape[1] % 32 == 0
    ), "model_image_size should be multiples of 32"

    # >>> WIP Add yolo-Fastest support

    if args.model_type.startswith("yolo_fastest"):
        from yolo_fastest.model import (
            get_yolo_fastest_train_model,
            yolo_fastest_data_generator_wrapper,
        )

        get_train_model = get_yolo_fastest_train_model
        data_generator = yolo_fastest_data_generator_wrapper
    ...
    ###if args.model_type.startswith("scaled_yolo4_") or args.model_type.startswith( "yolo5_"):
    elif args.model_type.startswith("scaled_yolo4_") or args.model_type.startswith(
        "yolo5_"
    ):
        ...
        tiny_version = False
    elif args.model_type.startswith("tiny_yolo3_") or args.model_type.startswith(
        "tiny_yolo4_"
    ):
        # elif num_anchors == 6:
    ...
    pruning_end_step = (
        np.ceil(1.0 * num_train / args.batch_size).astype(np.int32) * args.total_epoch
    )
    ...
    if args.decay_type or args.average_type:
        ...
        decay_steps = steps_per_epoch * (
            args.total_epoch - args.init_epoch - args.transfer_epoch
        )
        optimizer = get_optimizer(
            args.optimizer,
            args.learning_rate,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default=os.path.join("configs", "yolo3_anchors.txt"),
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default=os.path.join("configs", "voc_classes.txt"),
