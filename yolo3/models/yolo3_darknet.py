...
def depthwise_separable_resblock_body(x, num_filters, num_blocks):
    ...
    # Darknet uses left and top padding instead of 'same' mode
    x = ZeroPadding2D(((1, 0), (1, 0)))(x)
    x = Darknet_Depthwise_Separable_Conv2D_BN_Leaky(
        num_filters, (3, 3), strides=(2, 2)
    )(x)
    for i in range(num_blocks):
        y = compose(
            DarknetConv2D_BN_Leaky(num_filters // 2, (1, 1)),
...
def custom_yolo3_spp_body(inputs, num_anchors, num_classes, weights_path):
    ...
    y2 = DarknetConv2D(num_anchors * (num_classes + 5), (1, 1), name="predict_conv_2")(
        y2
    )
    y3 = DarknetConv2D(num_anchors * (num_classes + 5), (1, 1), name="predict_conv_3")(
        y3
    )
    return Model(inputs, [y1, y2, y3])
...
def tiny_yolo3_body_no_leaky(inputs, num_anchors, num_classes):
    ...
    y1 = compose(
        DarknetConv2D_BN(512, (3, 3), name="anchor_1"),
...
def tiny_yolo3_custom_leaky_body(inputs, num_anchors, num_classes):
    """Create Tiny YOLO_v3 model CNN body in keras."""
    # feature map 2 (26x26x256 for 416 input)
    f2 = compose(
        DarknetConv2D_BN_custom_Leaky(16, (3, 3)),
        MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding="same"),
        DarknetConv2D_BN_custom_Leaky(32, (3, 3)),
        MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding="same"),
        DarknetConv2D_BN_custom_Leaky(64, (3, 3)),
        MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding="same"),
        DarknetConv2D_BN_custom_Leaky(128, (3, 3)),
        MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding="same"),
        DarknetConv2D_BN_custom_Leaky(256, (3, 3)),
    )(inputs)

    # feature map 1 (13x13x1024 for 416 input)
    f1 = compose(
        MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding="same"),
        DarknetConv2D_BN_custom_Leaky(512, (3, 3)),
        MaxPooling2D(pool_size=(2, 2), strides=(1, 1), padding="same"),
        DarknetConv2D_BN_custom_Leaky(1024, (3, 3)),
    )(f2)

    # feature map 1 transform
    x1 = DarknetConv2D_BN_custom_Leaky(256, (1, 1))(f1)

    # feature map 1 output (13x13 for 416 input)
    y1 = compose(
        DarknetConv2D_BN_custom_Leaky(512, (3, 3), name = "conv2d_8"),
        DarknetConv2D(num_anchors * (num_classes + 5), (1, 1), name="predict_conv_1"),
    )(x1)
    ...
    y2 = compose(
        Concatenate(),
        DarknetConv2D_BN_custom_Leaky(256, (3, 3), name = "conv2d_11"),
        DarknetConv2D(num_anchors * (num_classes + 5), (1, 1), name="predict_conv_2"),
    )([x2, f2])
...
def custom_tiny_yolo3_body_no_leaky(inputs, num_anchors, num_classes, weights_path):
...
def custom_leaky_tiny_yolo3_body(inputs, num_anchors, num_classes, weights_path):
    """Create a custom Tiny YOLO_v3 model, use
    pre-trained weights from darknet and fit
    for our target classes."""
    # TODO: get darknet class number from class file
    num_classes_coco = 80
    base_model = tiny_yolo3_custom_leaky_body(inputs, num_anchors, num_classes_coco)
    base_model.load_weights(weights_path, by_name=False)
    print("Load weights {}.".format(weights_path))
    ...
    # y1 = base_model.get_layer('leaky_re_lu_8').output
    # y2 = base_model.get_layer('leaky_re_lu_10').output
    y1 = base_model.layers[62].output
    y2 = base_model.layers[63].output
    y1 = DarknetConv2D(num_anchors * (num_classes + 5), (1, 1), name="predict_conv_1")(
        y1
    )
    y2 = DarknetConv2D(num_anchors * (num_classes + 5), (1, 1), name="predict_conv_2")(
...
def custom_tiny_yolo4_body(
    ...
    model = Model(base_model.layers[1].input, [y1, y2])
...
BASE_WEIGHT_PATH = (
    "https://github.com/david8862/keras-YOLOv3-model-set/" "releases/download/v1.0.1/"
)
