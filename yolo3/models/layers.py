...
from common.backbones.layers import (
    YoloConv2D,
    YoloDepthwiseConv2D,
...
def Depthwise_Separable_Conv2D_BN_Leaky(filters, kernel_size=(3, 3), block_id_str=None):
    ...
    return compose(
        YoloDepthwiseConv2D(
            kernel_size, padding="same", name="conv_dw_" + block_id_str
        ),
        CustomBatchNormalization(name="conv_dw_%s_bn" % block_id_str),
        LeakyReLU(alpha=0.1, name="conv_dw_%s_leaky_relu" % block_id_str),
        YoloConv2D(
...
def DarknetConv2D_BN(*args, **kwargs):
    """Darknet Convolution2D followed by CustomBatchNormalization."""
    no_bias_kwargs = {"use_bias": False}
    no_bias_kwargs.update(kwargs)
    return compose(
        DarknetConv2D(*args, **no_bias_kwargs),
...
def Spp_Conv2D_BN_Leaky(x, num_filters):
    ...
    y = compose(Concatenate(), DarknetConv2D_BN_Leaky(num_filters, (1, 1)))(
        [y3, y2, y1, x]
    )
    return y
...
def make_depthwise_separable_last_layers(
    x, num_filters, out_filters, block_id_str=None, predict_filters=None, predict_id="1"
):
    """6 Conv2D_BN_Leaky layers followed by a Conv2D_linear layer"""
    if not block_id_str:
        block_id_str = str(K.get_uid())
...
def make_spp_depthwise_separable_last_layers(
    x, num_filters, out_filters, block_id_str=None, predict_filters=None, predict_id="1"
):
    """6 Conv2D_BN_Leaky layers followed by a Conv2D_linear layer"""
    if not block_id_str:
...
def yolo3_predictions(
    feature_maps, feature_channel_nums, num_anchors, num_classes, use_spp=False
):
    ...
    if use_spp:
        x, y1 = make_spp_last_layers(
            f1, f1_channel_num // 2, num_anchors * (num_classes + 5), predict_id="1"
        )
    else:
    ...
    x = compose(DarknetConv2D_BN_Leaky(f2_channel_num // 2, (1, 1)), UpSampling2D(2))(x)
    x = Concatenate()([x, f2])
    ...
    x = compose(DarknetConv2D_BN_Leaky(f3_channel_num // 2, (1, 1)), UpSampling2D(2))(x)
    x = Concatenate()([x, f3])
...
def yolo3lite_predictions(
    ...
    # feature map 1 head & output (13x13 for 416 input)
    if use_spp:
        x, y1 = make_spp_depthwise_separable_last_layers(
            f1,
            f1_channel_num // 2,
...
def tiny_yolo3_predictions(
    ...
    # upsample fpn merge for feature map 1 & 2
    x2 = compose(DarknetConv2D_BN_Leaky(f2_channel_num // 2, (1, 1)), UpSampling2D(2))(
        x1
    )
...
def tiny_yolo3lite_predictions(
    feature_maps, feature_channel_nums, num_anchors, num_classes
):
    f1, f2 = feature_maps
    f1_channel_num, f2_channel_num = feature_channel_nums
    ...
    y1 = compose(
        # DarknetConv2D_BN_Leaky(f1_channel_num, (3,3)),
        Depthwise_Separable_Conv2D_BN_Leaky(
            filters=f1_channel_num, kernel_size=(3, 3), block_id_str="pred_1"
        ),
        DarknetConv2D(num_anchors * (num_classes + 5), (1, 1), name="predict_conv_1"),
    )(x1)
    ...
    y2 = compose(
        ...
        # DarknetConv2D_BN_Leaky(f2_channel_num, (3,3)),
        Depthwise_Separable_Conv2D_BN_Leaky(
            filters=f2_channel_num, kernel_size=(3, 3), block_id_str="pred_2"
        ),
        DarknetConv2D(num_anchors * (num_classes + 5), (1, 1), name="predict_conv_2"),
    )([x2, f2])
