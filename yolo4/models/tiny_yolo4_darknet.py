...
def base_model(inputs,num_anchors):
    ...
    P5_output = DarknetConv2D(num_anchors*(num_classes_coco+5),(1,1),name = 'P5_out')(P5_output)
    P4_output = DarknetConv2D(num_anchors*(num_classes_coco+5),(1,1),name = 'P4_out')(P4_output)
...
def tiny_yolo4_body(inputs, num_anchors, num_classes, weights_path = None):
  ...
  P5_output = model_body.get_layer(name = 'P5_out').input
  P4_output = model_body.get_layer(name = 'P4_out').input
