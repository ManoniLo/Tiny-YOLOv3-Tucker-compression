...
def yolox_decode(feats, anchors, num_classes, input_shape, scale_x_y=None, calc_loss=False):
...
def yolox_correct_boxes(box_xy, box_wh, input_shape, image_shape):
    '''Get corrected boxes'''
...
def yolox_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape, scale_x_y):
    '''Process Conv layer output'''
    box_xy, box_wh, box_confidence, box_class_probs = yolox_decode(feats,
        anchors, num_classes, input_shape, scale_x_y=scale_x_y)
    boxes = yolox_correct_boxes(box_xy, box_wh, input_shape, image_shape)
...
def yolox_postprocess(args,
    ...
    for l in range(num_layers):
        _boxes, _box_scores = yolox_boxes_and_scores(yolo_outputs[l],
...
def batched_yolox_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape, scale_x_y):
    '''Process Conv layer output'''
    box_xy, box_wh, box_confidence, box_class_probs = yolox_decode(feats,
        anchors, num_classes, input_shape, scale_x_y=scale_x_y)
...
def batched_yolox_postprocess(args,
              anchors,
              num_classes,
              max_boxes=100,
              confidence=0.1,
    ...
    box_scores = []
    for l in range(num_layers):
        _boxes, _box_scores = batched_yolox_boxes_and_scores(yolo_outputs[l],
            anchors[anchor_mask[l]], num_classes, input_shape, image_shape, scale_x_y=scale_x_y[l])
        boxes.append(_boxes)
        box_scores.append(_box_scores)
...
class YoloxPostProcessLayer(Layer):
    def __init__(self, anchors, num_classes, input_dim, **kwargs):
        ...
        super(yoloxPostProcessLayer, self).__init__(**kwargs)
    ...
    def get_config(self):
        ...
        base_config = super(yoloxPostProcessLayer, self).get_config()
