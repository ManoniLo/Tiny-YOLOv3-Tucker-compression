...
def get_ground_truth_data(annotation_line, input_shape, augment=True, random_padd = True, letterbox_padd = False, max_boxes=100):
...
class YoloxDataGenerator(Sequence):
...
def yolox_data_generator(annotation_lines, batch_size, input_shape, anchors, num_classes, enhance_augment, rescale_interval, multi_anchor_assign,
    ...
    while True:
        ...
        for b in range(batch_size):
            ...
            image, box = get_ground_truth_data(annotation_lines[i], input_shape, augment=True, random_padd = False, letterbox_padd = False)
            image_data.append(image)
            box_data.append(box)
            i = (i+1) % n
        image_data = np.array(image_data)
...
def yolox_data_generator_wrapper(annotation_lines, batch_size, input_shape, anchors, num_classes, enhance_augment=None, rescale_interval=-1, multi_anchor_assign=False, **kwargs):
    n = len(annotation_lines)
    if n==0 or batch_size<=0: return None
    return yolox_data_generator(annotation_lines, batch_size, input_shape, anchors, num_classes, enhance_augment, rescale_interval, multi_anchor_assign)
