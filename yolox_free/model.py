...
from yolox_free.models.yolox_darknet import tiny_yolox_darknet
from yolox_free.loss import yolox_loss

from yolox_free.postprocess import (
    batched_yolox_postprocess,
    batched_yolox_prenms,
...
yolox_tiny_model_map = {"tiny_yolox_darknet": [tiny_yolox_darknet, 20, "weights/yolov3-tiny.h5"]}
...
def get_yolox_model(
    ...
    if num_feature_layers == 2:
        if model_type in yolox_tiny_model_map:
            model_function = yolox_tiny_model_map[model_type][0]
            backbone_len = yolox_tiny_model_map[model_type][1]
            weights_path = yolox_tiny_model_map[model_type][2]
    ...
    # YOLOv3 model has 9 anchors and 3 feature layers
        ...
        if model_type in yolox_model_map:
            model_function = yolox_model_map[model_type][0]
            backbone_len = yolox_model_map[model_type][1]
            weights_path = yolox_model_map[model_type][2]
...
def get_yolox_train_model(
    ...
    #]
    y_true = [
        Input(shape=(None, None, 1, num_classes + 5), name="y_true_{}".format(l))
        for l in range(num_feature_layers)
    ]
    

    model_body, backbone_len = get_yolox_model(
        model_type,
        num_feature_layers,
        num_anchors,
        num_classes,
    ...
    if freeze_level in [1, 2]:
        ...
        num = (backbone_len, len(model_body.layers) - 18)[freeze_level - 1]
        for i in range(num):
